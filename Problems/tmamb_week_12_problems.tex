...
\begin{prob}{25.4}
A sentence $X$ is a \emph{G\"odel sentence} of a set of numbers $\mathcal{S}$ when it is true if and only if its G\"odel number is in $\mathcal{S}$. Prove that every computable set has a G\"odel sentence. (Hint: If $n$ is the G\"odel number of $X$, then $n \ast 52$ is the G\"odel number of $X = t$. First prove that if $\mathcal{S}$ is computable, then the set $\mathcal{S}^*$ consisting of all $n$ such that $n \ast 52$ is in $\mathcal{S}$ is also computable.)
\end{prob}
